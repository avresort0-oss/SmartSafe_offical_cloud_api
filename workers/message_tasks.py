...
from .retry_utils import compute_backoff
...
def dispatch_whatsapp_message(self, message_id: str, target_phone: str = ""):
    ...
    try:
        ...
        if not resolved_phone:
             # Not retryable: no transient condition produces a phone number here.
             msg.status = "FAILED"
        ...
        logger.error(
            f"Attempt {retries_used + 1}/{self.max_retries + 1} failed for message {message_id}: {exc}"
        )
        if retries_used >= self.max_retries:
            # Retries exhausted. Celery's retry() re-raises `exc` itself in this
            # case (not MaxRetriesExceededError) because we pass exc= below for
            # logging -- so the terminal write has to happen here, not in an
            # except MaxRetriesExceededError clause that would never trigger.
            msg = db.query(Message).filter_by(id=message_id).first()
            if msg:
                msg.status = "FAILED"
                msg.provider_error_code = str(exc)[:64]
                db.commit()
        raise self.retry(exc=exc, countdown=compute_backoff(retries_used))
    finally:
        db.close()
...
def check_sla_breaches():
    ...
    try:
        ...
        db.rollback()
        logger.error(f"Error checking SLA breaches: {exc}")
        raise
    finally:
        db.close()
...
def process_scheduled_campaigns():
    ...
    try:
        ...
        db.rollback()
        logger.error(f"Error processing campaigns: {exc}")
        raise
    finally:
        db.close()
